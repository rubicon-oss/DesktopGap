...
namespace DesktopGap.Configuration.Security
{
  public class AddUrlConfigurationElement : UrlConfigurationElement
  {
    ...
    {
      return UseRegex
                 ? new PositiveUrlRule (Domain, Path, RequireSsl, IgnoreCase)
                 : new PositiveUrlRule (TranslateStartWildcard (Domain), TranslateEndWildcard (Path), RequireSsl, IgnoreCase);
    }
  }
}