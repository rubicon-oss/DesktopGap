...
namespace DesktopGap.Configuration.Security
{
  public class RemoveUrlConfigurationElement : UrlConfigurationElement
  {
    ...
    {
      return UseRegex
                 ? new NegativePositiveUrlRule (Domain, Path, RequireSsl, IgnoreCase)
                 : new NegativePositiveUrlRule (TranslateStartWildcard (Domain), TranslateEndWildcard (Path), RequireSsl, IgnoreCase);
    }
  }
}