...
namespace DesktopGap.Utilities.Web
{
  public class WebClientWrapper : IWebClient
  {
    private readonly WebClient _wrappedInstance;

    public WebClientWrapper (WebClient webClient)
    {
      ArgumentUtility.CheckNotNull ("webClient", webClient);
      _wrappedInstance = webClient;
    }

    public void DownloadFile (Uri uri, string file)
    {
      ArgumentUtility.CheckNotNull ("uri", uri);
      ArgumentUtility.CheckNotNull ("file", file);

      _wrappedInstance.DownloadFile (uri, file);
    }
  }
}